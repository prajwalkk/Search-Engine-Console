...
def remove_nodes(valid_websites, graph):
    nodes = graph.nodes()
    removable_nodes = [item for item in nodes if item.rstrip('/') not in valid_websites]
...
def add_edges(current_page, links, graph):
    for link in links:
        if graph.has_edge(current_page.rstrip('/'), link.rstrip('/')):
            ...
            graph.add_edge(current_page.rstrip('/'), link.rstrip('/'), weight=1)


def draw_graph(graph):
    nx.draw(graph, node_color="#0000AE", edge_cmap=plt.cm.Blues, with_labels=True)
    plt.show()
